    ...
    try:
        ...
        for element in clients:
            ...
            if cisco_id_client == 'Not Found':
                ...
                try:
                    query = f"SELECT * FROM dcs.pac_clients WHERE ip = '{ip}' AND device_cisco <> 'Not Found' ORDER BY id DESC LIMIT 1"
                    cursor.execute(query)
            ...
            query = (f"INSERT INTO dcs.pac_clients (name, description, ip, status_prtg, lastup_prtg, lastdown_prtg, device_ip_cisco, device_cisco, port_cisco, status_cisco, reachability_cisco, id_prtg, status_device_cisco, data_backup)"
                f"VALUES ('{name}', '{description}', '{ip}', '{status_prtg}', '{last_up_prtg}', '{last_down_prtg}', '{cisco_device_ip_adrress}', '{cisco_device_name}', '{cisco_client_port}', '{cisco_client_status}', '{cisco_device_reachability}', '{id_device_prtg}', '{prtg_device_status}', '{is_databackup}')")
            cursor.execute(query)
            mydb.commit()
